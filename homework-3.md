...
SELECT COUNT(*) FROM `practica-cloud-485603.taxi.yellow_tripdata_2024_materialized` 
...
```sql
SELECT DISTINCT(PULocationID) FROM `practica-cloud-485603.taxi.external_yellow_tripdata_2024` 
```
Answer = 0 B
```sql
SELECT DISTINCT(PULocationID) FROM `practica-cloud-485603.taxi.yellow_tripdata_2024_materialized` 
```
Answer = 155.12 MB
...
```sql
SELECT PULocationID 
FROM `practica-cloud-485603.taxi.yellow_tripdata_2024_materialized`;
```
155.12 MB
...
```sql
CREATE OR REPLACE TABLE `practica-cloud-485603.taxi.yellow_tripdata_2024_partitioned`
PARTITION BY DATE(tpep_dropoff_datetime)
AS
SELECT * FROM `practica-cloud-485603.taxi.yellow_tripdata_2024_materialized`;
```
...
```sql
SELECT DISTINCT VendorID
FROM `practica-cloud-485603.taxi.yellow_tripdata_2024_partitioned`
WHERE tpep_dropoff_datetime BETWEEN '2024-03-01' AND '2024-03-15 23:59:59';
```
Answer = 26.84 MB
...
BigQuery stores table metadata that includes the total number of rows  The count is already precalculated in the metadata  It doesn't need to scan any columns or rows to answer.
